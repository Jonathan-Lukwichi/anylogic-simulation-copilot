...
| [.claude/skills/](.claude/skills/) | Fourteen skills: seven AnyLogic skills (DES, ABM, SD, 2D, 3D, libraries, debug) encoding idioms and root-caused bugs from real builds; `simulation-welcome` (guided intake) and `simulation-build-plan` (step-by-step plan with preview flow diagram and canvas layout); `stats-analyst` and `supply-chain-analyst` (plain-English interpretation of scenarios and results); `context-distiller` (two-stage hygiene for messy inputs), `model-premortem` (FMEA guardrails on every build plan), and `triangulate-verify` (multi-source truth verification for claims). The debug and premortem skills **self-update**: every new root cause is recorded, so the system sharpens with use |
