...
| [.claude/skills/](.claude/skills/) | Eleven skills: seven AnyLogic skills (DES, ABM, SD, 2D, 3D, libraries, debug) encoding idioms and root-caused bugs from real builds; `simulation-welcome` (guided project intake) and `simulation-build-plan` (step-by-step build plan with a preview flow diagram and canvas layout); `stats-analyst` and `supply-chain-analyst` (plain-English interpretation of scenarios and uploaded results, grounded in the industrial-statistics and supply-chain canon). The debug skill **self-updates**: every new root cause is recorded, so the system sharpens with use |
